...
/**
 * Notification data for earned achievements
 */
...
export class BadgeUpStoredEvent {
     ...
     */
    public badgeUpEvent: BadgeUpEvent;

    constructor(event: BadgeUpEvent, id?: string) {
        this.badgeUpEvent = event;
        this.id = id || '' + (Math.floor(Math.random() * 1e6));
    }
}
...
export interface BadgeUpStorage {
     ...
     */
    storeEvent(badgeUpEvent: BadgeUpEvent): void;
     ...
     * on each event.
     */
    removeEvents(badgeUpEvents: BadgeUpStoredEvent[]): void;
}
